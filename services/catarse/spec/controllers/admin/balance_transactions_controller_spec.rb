    ...
    context 'when user has admin roles' do
      ...
      context 'when successfully transferred' do
        before do
          ...
          post :transfer_balance, balance_transaction: { from_user_id: from_user.id, to_user_id: to_user.id, amount: amount }
        end
      ...
      context 'when an error occurs' do
        before { post :transfer_balance, balance_transaction: { from_user_id: from_user.id, to_user_id: to_user.id, amount: amount } }

        it 'doesn`t transfer the amount to receiver' do
          expect(to_user.total_balance).to eq(0)
        end

        it 'returns `bad_request` http response' do
